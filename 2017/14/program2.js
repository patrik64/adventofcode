...
function markNeighbors(mtx, i, j)
{
    mtx[i][j] = -1;
    if((i-1) >= 0)
        if(mtx[i-1][j] == 1)
            markNeighbors(mtx, i-1, j);
    if((j-1) >= 0)
        if(mtx[i][j-1] == 1)
            markNeighbors(mtx, i, j-1);
    if((j+1) < mtx.length)
        if(mtx[i][j+1] == 1)
            markNeighbors(mtx, i, j+1);
    if((i+1) < mtx.length)
...
var regions = calculateRegions(arr);
console.log(regions);  
