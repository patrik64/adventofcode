...
function messageToString(message, aNr) {
    let str = '';
...
let sum = 0;
let rTree = generateTreeForRule(0, rules, aNr, bNr);
let gm = traverseTree(rTree);
...
for(let m in converted) {
    ...
    console.log('checking message [', m, '] of ', len, ' -->', messageToString(message['arr'], aNr));
    if(arrayIn(message['arr'], gm)) {
        message['found'] = true;
    }
}
